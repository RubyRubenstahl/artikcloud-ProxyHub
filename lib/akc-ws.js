    ...
    if (data) {
      ...
      if (json.type !== 'ping') {
        if (json.data && json.data.cid) {
          ...
          if ( (akcDeviceId != null) && (json.error.message === 'Please provide a valid authorization header' || json.error.message === 'Device(s) not found')) {
            logger.error('ARTIK Cloud device (id = ' + akcDeviceId + ')does not exist we delete it')
            self.emit('unlinkAkcDevice', akcDeviceId)
          } else if (akcDeviceId != null && json.error.message === 'Device not registered') {
            logger.error('Error sending msg we reregister the device: %s', data)
            logger.error('Error sending msg we reregister the device: %s', data)
            self.unregisterDevice(akcDeviceId)
            self.registerDevice(akcDeviceId)
...
AKCWS.prototype.unregisterDevice = function (sdid) {
  if (sdid in this._devices) {
    logger.debug('Unregistering device '+sdid)
