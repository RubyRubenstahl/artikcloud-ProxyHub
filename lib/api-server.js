...
ApiServer.prototype._initProxiesRouter = function () {
  ...
  this._proxiesRouter.post('/setupApplication', function (req, res) {
    ...
    if(self._config.optionalArtikOnBoarding.userId === '' || self._config.optionalArtikOnBoarding.clientId === '' || self._config.optionalArtikOnBoarding.clientSecret === '')
      self.emit('setupApp', req.body, callbackSetupDone)
    else {
      var userToken = _findToken(req)
      var userId = req.body.userId || req.session.userId
      _checkTokenAndUserId(userToken, userId, self._config.optionalArtikOnBoarding)
        .then(self.emit.bind(self, 'setupApp', req.body, callbackSetupDone))
        .catch(function (reason) {
          var resJson = { 'errorCode': reason.statusCode, 'errorMessage': reason.error }
          res.status(reason.statusCode).json(resJson)
        })
  ...
  this._proxiesRouter.post('/newplugin', function (req, res) {
    logger.debug('post', '/proxies/newplugin')
    logger.debug('request.session', req.session)
    var callbackPluginUploaded = function (result) {
      if('error' in result)
      {
        res.status(400).json(result)
      }
    ...
    var userId = req.body.userId || req.session.userId
    _checkTokenAndUserId(userToken, userId, self._config.optionalArtikOnBoarding)
      .then(self.emit.bind(self, 'newPlugin', req, callbackPluginUploaded))
      .catch(function (reason) {
        var resJson = { 'errorCode': reason.statusCode, 'errorMessage': reason.error }
        res.status(reason.statusCode).json(resJson)
      })
...
ApiServer.prototype.listenTo = function (consumer) {
  if (consumer) {
    ...
    consumer.on('proxiesUpdate', (function () {
      if(this._socket != null) {
        this._socket.emit('proxiesUpdate')
      }
    }).bind(this))
  }
}
