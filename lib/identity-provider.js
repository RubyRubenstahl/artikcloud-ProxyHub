...
IdentityProvider.init = function (config) {
  ...
  var passportOptions = {
    authorizationURL: IdentityProvider._config.optionalArtikOnBoarding.authUrl + '/authorize?prompt=login',
  ...
  IdentityProvider.get('/logout', function (req, res) {
    ...
    return new Promise(function () {
      ...
      var options = {
        uri: IdentityProvider._config.optionalArtikOnBoarding.authUrl+'/revokeToken',
        method: 'POST',
        headers: {
      ...
      request(options, function(err, res) {
        if(err){
          ...
          response.redirect('/#error=Fail to log out. Please try again later.')
          return
        }
        ...
        response.redirect('/#success=You have been successfully logged out.')
      })
    })
  })
}
