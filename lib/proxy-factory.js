...
ProxyFactory.prototype.listenTo = function (eventEmitter) {
  ...
  eventEmitter.on('removeProxyInFactory', function (proxyName) {
    logger.debug('Remove proxy', proxyName)
    self._removeProxy(proxyName)
  })
  ...
  eventEmitter.on('switchProxyInFactory', function (params, callback) {
    logger.debug('switchProxyInFactory')
    self._onSwitchProxy(params, callback)
  })
...
var extractInfoProxy = function (fname) {
  ...
  if(this._proxies[proxyName != null])
...
ProxyFactory.prototype._onSwitchProxy = function (params, callback) {
  // callback(err, proxyInfo)
  var self = this
  var proxyName = params.proxyName.toString().split('_').pop()
  var enabled = params.enabled
  var newProxyName = (enabled ? '' : '_') + proxyName
  var originProxyName = (enabled ? '_' : '') + proxyName
  var newProxyPath = Path.join(self._config.dir.proxies, newProxyName)
  var originProxyPath = Path.join(self._config.dir.proxies, originProxyName)
  var renameProxy = function () {
    Fs.rename(originProxyPath, newProxyPath, function (err) {
      if (err) {
        callback(err)
      } else {
        callback(null, extractInfoProxy.bind(self)(newProxyName))
      }
    })
  }
  Fs.access(newProxyPath, function (err) {
    if (err) {
      renameProxy()
      logger.error('Could not access the proxy path %s: %s', newProxyPath, err)
    } else {
      callback(null, extractInfoProxy.bind(self)(newProxyName))
    }
  })
...
ProxyFactory.prototype._onUpdateProxyConfig = function (params, callback) {
  ...
  var enabledProxyPath = Path.join(self._config.dir.proxies, proxyName)
  var disabledProxyPath = Path.join(self._config.dir.proxies, '_' + proxyName)
  ...
  Fs.access(enabledProxyPath, function (err) {
    if (err) {
      Fs.access(disabledProxyPath, function (err) {
        if (err) {
          callback(err)
        } else {
          updateProxyConfig('_' + proxyName)
        }
      })
      logger.error('Could not access the proxy path %s: %s', enabledProxyPath, err)
    } else {
...
ProxyFactory.prototype._onNewProxyDevice = function (proxyName, proxyDevice) {
  ...
  proxyDevice.isVirtual = this._proxies[proxyName].addNewDevice != null
...
ProxyFactory.prototype._scheduleUpdate = function (scheduleUpdatePeriodMs, proxyName) {
  ...
  if (!this._closed) {
    this._scheduledUpdateTimeouts[proxyName] = setTimeout(function () {
      ...
      if (proxy != null && proxy.scheduledUpdate) {
