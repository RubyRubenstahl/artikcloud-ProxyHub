...
AKCWrapper.prototype.init = function (config) {
  ...
  //init disabledProxies
  this._disabledProxies = this._loadDataFromFile('disabledProxies.json')
  if(this._disabledProxies == null)
    this._disabledProxies = []
...
AKCWrapper.prototype.listenTo = function (producer, consumer) {
  ...
  if (consumer) {
    ...
    consumer.on('getNotLinkedDevices', function (callback) {
      ...
      if (callback != null ) {
        var filteredNotLinkedProxyDevices = {}
        for(var index in self._notLinkedProxyDevices){
          var item = self._notLinkedProxyDevices[index]
          var deviceProxyName = parseUniqueProxyDeviceId(index)[0]
          if (self._disabledProxies.indexOf(deviceProxyName) <= 0) {
            filteredNotLinkedProxyDevices[index] = item
          }
        }
        callback(null, filteredNotLinkedProxyDevices)
      }
    })
    ...
    consumer.on('getProxies', function (callback) {
      ...
      if (callback) {
        self.emit('getProxiesFromFactory', function(listProxies){
          logger.debug('filter proxies from the factory')
          var filteredListProxies = listProxies.map(function (el) {
            el.enabled = (self._disabledProxies.indexOf(el.name) == -1)
            return el
          })
          callback(filteredListProxies)
        })
      }
    })
    consumer.on('switchProxy', function (params, callback) {
      logger.debug('Receive request to switchProxy')
      params.returnCallback = callback
      self.emit('getProxyInfoFromFactory', params, self.switchProxy.bind(self))
    })
...
AKCWrapper.prototype.switchProxy = function (params) {
  logger.debug('Switching proxy')
  if (params.proxyInfo != null) {
    var proxyIndex = this._disabledProxies.indexOf(params.proxyName)
    if (params.enabled) {
      // if request to enable the proxy, remove it from disabled proxy if necessary
      if (proxyIndex >= 0) {
        this._disabledProxies.splice(proxyIndex,1)
        this._saveDataInFile('disabledProxies.json', this._disabledProxies)
      }
    } else {
      // if request to disable the proxy, add it to disabled proxy if necessary
      if (proxyIndex < 0) {
        this._disabledProxies.push(params.proxyName)
        this._saveDataInFile('disabledProxies.json', this._disabledProxies)
      }
    }
    this.emit('notLinkedProxyDevicesUpdate')
    if (params.returnCallback) {
      params.returnCallback(null, params.proxyInfo)
    }
  } else {
    if (params.returnCallback) {
      params.returnCallback('Unknown proxy name ' + params.proxyName)
    }
  }
}
...
AKCWrapper.prototype._onGetStatus = function (callback) {
  ...
  var statusesCallback = function(statuses)
  {
    ...
    for(var i in statuses.proxiesStatuses)
    {
      ...
      if(self._disabledProxies.indexOf(status.proxyName) > -1)
        continue
...
AKCWrapper.prototype._deleteAKCDevice = function (proxyDeviceId, callback) {
  ...
  var akcDeletionDone = function akcDeletionDone (error, data, response) {
    if (error) {
      logger.error('Error deleteing device for proxy %s\n   -> Error: %s\n   -> Response: %s', JSON.stringify(self._linkedProxyDevices[proxyDeviceId]), error, response.text)
    } else {
      if (self._linkedProxyDevices[proxyDeviceId]['akcDeviceId']) {
