...
/***/ function(module, exports, __webpack_require__) {
	...
	var Helper = {
	  timestamp2Datetime: function timestamp2Datetime(ts) {
	    if (ts) {
	  ...
	  Fetcher: function Fetcher(config) {
	    ...
	    config = {
	      ...
	      callbackOK(response): FUNCTION_CALLBACK_OK,
	      callbackNotOK(response): FUNCTION_CALLBACK_NOT_OK
	    }
	    */
	    var result = {
	      responseHandler: {},
	    ...
	    result.errorHandler = function (error) {
	      console.error('There has been a problem with your fetch operation when', config.fetchConfig.method, config.url, error.message);
	    };
	    result.responseHandler = function (response) {
	      console.dir(response);
	      if (response.ok) {
	        console.info(config.fetchConfig.method, config.url, 'Response was OK.');
	        if (config.callbackOK) {
	          config.callbackOK(response);
	        }
	      } else {
	        console.info(config.fetchConfig.method, config.url, 'Response was not OK.');
	        console.warn(response.status, response.statusText);
	        if (config.callbackNotOK) {
	          config.callbackNotOK(response);
	        } else {
	    ...
	    result.doFetch = function () {
	      window.fetch(config.url, config.fetchConfig).then(result.responseHandler).catch(result.errorHandler);
	    };
	    return result;
	  }
	};
	...
	var Actions = _reflux2.default.createActions(['updateLoginStatus', 'refreshList', 'loadDevices', 'loadProxies', 'switchProxy', 'updateProxyConfig', 'loadCandidates', 'createAKCDevice', 'linkAKCDevice']);

	var Store = _reflux2.default.createStore({
	  // Initial setup
	  init: function init() {
	    console.log('Reflux store init');
	    Actions.updateLoginStatus();
	  },
	  listenables: Actions,
	  onLinkAKCDevice: function onLinkAKCDevice(proxyDeviceId, akcDeviceId, akcDeviceName) {
	    var fetchConfig = {
	      method: 'POST',
	      headers: { 'Content-Type': 'application/json' },
	      credentials: 'include',
	      body: JSON.stringify({
	        akcDeviceId: akcDeviceId,
	        akcDeviceName: akcDeviceName
	      })
	    };
	    var handleResponseOK = function handleResponseOK(response) {
	      console.info('POST', '/proxydevices/:proxyDeviceId', 'Response was OK.');
	      response.json().then(function (json) {
	        console.log('LinkAKCDevice success with data:', json);
	        Actions.refreshList();
	      });
	    };
	    var fetcher = Helper.Fetcher({
	      url: '/proxydevices/' + proxyDeviceId,
	      fetchConfig: fetchConfig,
	      callbackOK: handleResponseOK
	    });
	    fetcher.doFetch();
	  },
	  onCreateAKCDevice: function onCreateAKCDevice(proxyDeviceId, inputAkcDeviceName) {
	    var fetchConfig = {
	      method: 'POST',
	      headers: { 'Content-Type': 'application/json' },
	      credentials: 'include',
	      body: JSON.stringify({
	        akcDeviceName: inputAkcDeviceName || proxyDeviceId
	      })
	    };
	    var handleResponseOK = function handleResponseOK(response) {
	      response.json().then(function (json) {
	        console.log('CreateAKCDevice success with data:', json);
	        Actions.refreshList();
	      });
	    };
	    var fetcher = Helper.Fetcher({
	  ...
	  onLoadDevices: function onLoadDevices(optionStr) {
	    console.log('onLoadDevices in', 'Store');
	    var self = this;
	    var fetchConfig = {
	      method: 'GET',
	    ...
	    var handleResponseOK = function handleResponseOK(response) {
	      response.json().then(function (chunk) {
	        console.log('load', optionStr, 'Devices success with data:', chunk);
	        if (optionStr === 'notlinked') {
	          storeData.notlinkedDevices = chunk.data.devices;
	        }
	        if (optionStr === 'linked') {
	          storeData.linkedDevices = chunk.data.devices;
	        }
	        self.trigger(storeData);
	      });
	    };
	    var fetcher = Helper.Fetcher({
	      url: '/proxydevices/' + optionStr,
	      fetchConfig: fetchConfig,
	      callbackOK: handleResponseOK
	    });
	    fetcher.doFetch();
	  },
	  onSwitchProxy: function onSwitchProxy(proxyName, enabled) {
	    var fetchConfig = {
	      method: 'PUT',
	      headers: { 'Content-Type': 'application/json' },
	      credentials: 'include',
	    ...
	    var handleResponseOK = function handleResponseOK(response) {
	      response.json().then(function (json) {
	        console.log('SwitchProxy success with data:', json);
	        Actions.loadProxies();
	      });
	    };
	    var fetcher = Helper.Fetcher({
	      url: '/proxies/' + proxyName,
	      fetchConfig: fetchConfig,
	      callbackOK: handleResponseOK
	    });
	    fetcher.doFetch();
	  },
	  onUpdateProxyConfig: function onUpdateProxyConfig(proxyName, config) {
	    var fetchConfig = {
	      method: 'PUT',
	      headers: { 'Content-Type': 'application/json' },
	      credentials: 'include',
	    ...
	    var handleResponseOK = function handleResponseOK(response) {
	      response.json().then(function (json) {
	        console.log('UpdateProxyConfig success with data:', json);
	        Actions.loadProxies();
	      });
	    };
	    var fetcher = Helper.Fetcher({
	      url: '/proxies/' + proxyName + '/customConfig',
	      fetchConfig: fetchConfig,
	      callbackOK: handleResponseOK
	    });
	    fetcher.doFetch();
	  },
	  onLoadProxies: function onLoadProxies() {
	    var self = this;
	    var fetchConfig = {
	      method: 'GET',
	      headers: { 'Accept': 'application/json' },
	    ...
	    var handleResponseOK = function handleResponseOK(response) {
	      response.json().then(function (chunk) {
	        storeData.proxies = chunk.data.proxies;
	        self.trigger(storeData);
	      });
	    };
	    var fetcher = Helper.Fetcher({
	  ...
	  onRefreshList: function onRefreshList() {
	    console.log('onRefreshList in', 'Store');
	    Actions.loadDevices('notlinked');
	    Actions.loadDevices('linked');
	    // this.trigger(storeData)
	  },
	  onUpdateLoginStatus: function onUpdateLoginStatus() {
	    ...
	    var handleResponseOK = function handleResponseOK(response) {
	      response.json().then(function (json) {
	        console.log('get login status success with:', json);
	        storeData.loginStatus = json.state ? json.state : false;
	        self.trigger(storeData);
	      });
	    };
	    var fetcher = Helper.Fetcher({
	      url: '/auth/loginstatus',
	      fetchConfig: fetchConfig,
	      callbackOK: handleResponseOK
	    });
	    fetcher.doFetch();
	...
	var CandidateAKCDevicesList = _react2.default.createClass({
	  displayName: 'CandidateAKCDevicesList',

	  handleSelectedChange: function handleSelectedChange(event) {
	  ...
	  loadCandidates: function loadCandidates() {
	    ...
	    var handleResponseOK = function handleResponseOK(response) {
	      response.json().then(function (chunk) {
	        console.log('loadCandidates success with data:', chunk);
	        self.setState({
	          data: chunk.data
	        });
	        var candidates = chunk.data.candidates;
	        var _defaultValue = JSON.stringify(candidates[0]);
	        console.log('_defaultValue', _defaultValue);
	        self.props.onSelectedChange(_defaultValue);
	        console.log('candidates.length', candidates.length);
	        if (candidates.length > 0) {
	          self.props.onHiddenChange(false);
	        } else {
	          self.props.onHiddenChange(true);
	        }
	      });
	    };
	    var fetcher = Helper.Fetcher({
	      url: '/proxydevices/' + this.props.proxyDeviceId + '/candidates',
	      fetchConfig: fetchConfig,
	      callbackOK: handleResponseOK
	    });
	    fetcher.doFetch();
	  ...
	  render: function render() {
	    console.log('rendering CandidateAKCDevicesList');
	    var candidateNodes = [];
	    if (this.state.data.candidates) {
	      candidateNodes = this.state.data.candidates.map(function (candidate, index) {
	        return _react2.default.createElement(
	          'option',
	          { key: index, value: JSON.stringify(candidate) },
	          candidate.akcDeviceName
	        );
	      });
	    }
	    return _react2.default.createElement(
	      'select',
	      { className: 'form-control candidateAKCDevicesList', onChange: this.handleSelectedChange },
	      candidateNodes
	    );
	  }
	});

	var LinkAKCDeviceForm = _react2.default.createClass({
	  displayName: 'LinkAKCDeviceForm',

	  handleSelectedChange: function handleSelectedChange(optionState) {
	    console.log('received optionState', optionState);
	    if (optionState) {
	      this.setState({
	        akcDeviceId: JSON.parse(optionState).akcDeviceId,
	        akcDeviceName: JSON.parse(optionState).akcDeviceName
	      });
	    }
	  },
	  handleHiddenChange: function handleHiddenChange(hiddenFlag) {
	    console.log('received isHidden', hiddenFlag);
	    this.setState({
	      isHidden: hiddenFlag
	    });
	  },
	  handleSubmit: function handleSubmit(event) {
	    ...
	    Actions.linkAKCDevice(this.props.proxyDeviceId, this.state.akcDeviceId, this.state.akcDeviceName);
	  },
	  getInitialState: function getInitialState() {
	    return {
	      akcDeviceId: null,
	  ...
	  render: function render() {
	    console.log('rendering LinkAKCDeviceForm');
	    var shouldHide = this.state.isHidden ? ' hidden' : '';
	    return _react2.default.createElement(
	      'li',
	      { className: 'well' + shouldHide },
	      'Or link it with an existing ARTIK Cloud device',
	      _react2.default.createElement(
	        'form',
	        { className: 'linkAKCDeviceForm', onSubmit: this.handleSubmit },
	        _react2.default.createElement(
	          'div',
	          ...
	          _react2.default.createElement(CandidateAKCDevicesList, { proxyDeviceId: this.props.proxyDeviceId, onSelectedChange: this.handleSelectedChange, onHiddenChange: this.handleHiddenChange })
	        ),
	        _react2.default.createElement(
	          'button',
	...
	var CreateAKCDeviceForm = _react2.default.createClass({
	  displayName: 'CreateAKCDeviceForm',

	  handleNameChange: function handleNameChange(event) {
	    this.setState({ inputAkcDeviceName: event.target.value });
	  },
	  handleSubmit: function handleSubmit(event) {
	    event.preventDefault();
	    Actions.createAKCDevice(this.props.proxyDeviceId, this.state.inputAkcDeviceName);
	  },
	  getInitialState: function getInitialState() {
	    return { inputAkcDeviceName: this.props.proxyDeviceName };
	  },
	  render: function render() {
	    console.log('rendering CreateAKCDeviceForm');
	    return _react2.default.createElement(
	      'li',
	      { className: 'well' },
	      'Create a new ARTIK Cloud device',
	      _react2.default.createElement(
	        'form',
	        { className: 'createAKCDeviceForm', onSubmit: this.handleSubmit },
	        _react2.default.createElement(
	          'div',
	          { className: 'form-group' },
	          _react2.default.createElement(
	          ...
	          _react2.default.createElement('input', {
	            ...
	            type: 'text',
	            value: this.state.inputAkcDeviceName,
	            placeholder: this.props.proxyDeviceName,
	...
	var GoodForms = function GoodForms(props) {
	  return _react2.default.createElement(
	    'div',
	    { className: 'goodForms' },
	    _react2.default.createElement(
	      'ul',
	      null,
	      _react2.default.createElement(CreateAKCDeviceForm, { key: 'create' + props.device.proxyDeviceId, proxyDeviceId: props.device.proxyDeviceId, proxyDeviceName: props.device.proxyDeviceName }),
	      _react2.default.createElement(LinkAKCDeviceForm, { key: 'link' + props.device.proxyDeviceId, proxyDeviceId: props.device.proxyDeviceId })
	    )
	  );
	};

	var BadForms = function BadForms(props) {
	  return _react2.default.createElement(
	    'div',
	    { className: 'badForms' },
	    _react2.default.createElement(
	    ...
	    _react2.default.createElement(
	      'a',
	      { className: 'btn-link', href: '/auth/artikcloud' },
	      ' login '
	    ),
	    _react2.default.createElement(
	      'span',
	      null,
	      'to create or link this proxy devices.'
	    )
	  );
	};

	var DeviceContent = _react2.default.createClass({
	  displayName: 'DeviceContent',

	  mixins: [_reflux2.default.connect(Store, 'storeKey'), Helper],
	  render: function render() {
	    var notlinkedForm = this.state.storeKey.loginStatus ? _react2.default.createElement(GoodForms, { device: this.props.device }) : _react2.default.createElement(BadForms, null);
	    var form = this.props.linked ? null : notlinkedForm;
	    var akcDeviceNameItem = this.props.linked ? _react2.default.createElement(
	      'li',
	      null,
	      'name: ',
	      this.props.device.akcDeviceName
	    ) : null;
	    var akcDeviceIdItem = this.props.linked ? _react2.default.createElement(
	      'li',
	      null,
	      'id: ',
	      this.props.device.akcDeviceId
	    ) : null;
	    var akcDeviceList = _react2.default.createElement(
	      'ul',
	      null,
	      akcDeviceNameItem,
	      akcDeviceIdItem,
	      _react2.default.createElement(
	        ...
	        null,
	        'last message: ',
	        this.props.device.lastMessageTS ? this.timestamp2Datetime(this.props.device.lastMessageTS) : 'Never'
	      ),
	      _react2.default.createElement(
	        'li',
	        null,
	        'last action: ',
	        this.props.device.lastActionTS ? this.timestamp2Datetime(this.props.device.lastActionTS) : 'Never'
	      )
	    );
	    return _react2.default.createElement(
	      'div',
	      { className: 'deviceContent panel-body' },
	      !form && akcDeviceList,
	      form
	    );
	  }
	...
	var OneDevice = function OneDevice(props) {
	  console.log('rendering OneDevice');
	  var panelType = props.device.found === true ? props.linked ? 'panel-info' : 'panel-primary' : 'panel-danger';
	  var className = 'oneDevice panel panel-inline ' + panelType;
	  return _react2.default.createElement(
	    'div',
	    { className: className },
	    _react2.default.createElement(
	      ...
	      props.device.proxyDeviceName,
	      ' (type:',
	      props.device.proxyDeviceTypeName,
	      ')'
	    ),
	    _react2.default.createElement(DeviceContent, { device: props.device, linked: props.linked })
	  );
	};

	var DevicesList = _react2.default.createClass({
	  displayName: 'DevicesList',

	  mixins: [_reflux2.default.connect(Store, 'storeKey'), Helper],
	  componentDidMount: function componentDidMount() {
	    var linkedFlagStr = this.props.linked ? 'linked' : 'notlinked';
	    Actions.loadDevices(linkedFlagStr);
	  },
	  render: function render() {
	    console.log('rendering DevicesList');
	    console.log('DevicesList.state.storeKey', this.state.storeKey);
	    var linked = this.props.linked;
	    var linkedStr = linked ? 'Registered devices' : 'New devices';
	    var devices = linked ? this.state.storeKey.linkedDevices : this.state.storeKey.notlinkedDevices;
	    var title = devices.length !== 0 ? linkedStr : 'No ' + linkedStr;
	    var className = devices.length === 0 ? 'text-muted' : '';
	    var deviceNodes = 'No ' + linkedStr;
	    if (devices) {
	      deviceNodes = devices.map(function (device, index) {
	        return _react2.default.createElement(OneDevice, { device: device, key: index, linked: linked });
	      });
	    }
	    return _react2.default.createElement(
	      'div',
	...
	var ProxySwitch = _react2.default.createClass({
	  displayName: 'ProxySwitch',

	  handleChange: function handleChange(event) {
	    Actions.switchProxy(this.props.proxyName, event.target.checked);
	  },
	  render: function render() {
	    return _react2.default.createElement('input', { type: 'checkbox', checked: this.props.enabled ? 'checked' : null, onChange: this.handleChange });
	  }
	});

	/*
	const OneProxy = React.createClass({
	  except: function (obj, exceptKeys) {
	    const copy = Object.assign({}, obj)
	    exceptKeys.forEach(k => delete copy[k])
	    return copy
	  },
	  handleNameChange: function (name, event) {
	    const copy = Object.assign({}, this.state.proxy)
	  ...
	  handleSubmit: function (event) {
	    event.preventDefault()
	    Actions.updateProxyConfig(this.props.proxy.proxyName, this.state.proxy)
	  },
	  getInitialState: function () {
	    return {proxy: this.except(this.props.proxy, ['enabled', 'proxyName'])}
	  },
	  render: function () {
	    const self = this
	    const proxyMutableProps = this.except(this.props.proxy, ['enabled', 'proxyName'])
	    const proxyProps = Object.keys(proxyMutableProps)
	      .map(function (prop, index) {
	        return <li key={index}>
	                 ...
	                 <input
	                   className='form-control'
	                   type='text'
	                   value={self.state.proxy[prop]}
	                   placeholder={self.props.proxy[prop]}
	                   onChange={self.handleNameChange.bind(self, prop)} />
	               </li>
	      })
	    return (
	      <div className='oneDevice panel panel-inline panel-info'>
	        <div className='panel-heading'>
	          {this.props.proxy.proxyName}
	        </div>
	        <form className='panel-body updateProxyConfigForm' onSubmit={this.handleSubmit}>
	          <ul className='form-group'>
	            <li>
	              {'enabled: '}
	              <ProxySwitch enabled={this.props.proxy.enabled} proxyName={this.props.proxy.proxyName} />
	            </li>
	            <li>
	              {`proxyName: ${this.props.proxy.proxyName}`}
	            </li>
	            {proxyProps}
	          </ul>
	          <button type='submit' className='form-control btn btn-link'>
	            Update
	          </button>
	        </form>
	      </div>
	...
	*/
	...
	var OneProxy = _react2.default.createClass({
	  displayName: 'OneProxy',

	  except: function except(obj, exceptKeys) {
	    var copy = Object.assign({}, obj);
	    exceptKeys.forEach(function (k) {
	      return delete copy[k];
	    });
	    return copy;
	  },
	  handleNameChange: function handleNameChange(name, event) {
	    var copy = Object.assign({}, this.state.proxy);
	    copy[name] = event.target.value;
	    this.setState({
	      proxy: copy
	    });
	  ...
	  render: function render() {
	    var self = this;
	    var proxyMutableProps = this.except(this.props.proxy, ['enabled', 'proxyName']);
	    return _react2.default.createElement(
	      'li',
	      { className: 'oneProxy checkbox' },
	      _react2.default.createElement(
	        'label',
	        null,
	        _react2.default.createElement(ProxySwitch, { enabled: this.props.proxy.enabled, proxyName: this.props.proxy.name }),
	        this.props.proxy.displayName
	      )
	    );
	  }
	});

	var ProxiesList = _react2.default.createClass({
	  displayName: 'ProxiesList',
	  ...
	  render: function render() {
	    console.log('DevicesList.state.storeKey', this.state.storeKey);
	    var proxies = this.state.storeKey.proxies;
	    var proxyNodes = 'No proxies';
	    var compareStr = function compareStr(a, b) {
	      var aStr = a['displayName'].toLowerCase();
	      var bStr = b['displayName'].toLowerCase();
	      if (aStr < bStr) {
	        return -1;
	      }
	      if (aStr > bStr) {
	    ...
	    if (proxies) {
	      proxyNodes = proxies.sort(compareStr).map(function (proxy) {
	        return _react2.default.createElement(OneProxy, { proxy: proxy, key: proxy.name });
	      });
	    }
	    return _react2.default.createElement(
	      'div',
	      { className: 'proxiesList' },
	      _react2.default.createElement(
	        'h3',
	        null,
	        'Proxies'
	...
	var ContentBox = _react2.default.createClass({
	  ...
	  mixins: [_reflux2.default.connect(Store, 'storeKey'), Helper],
	  render: function render() {
	    console.log('rendering ContentBox');
	    var words = this.state.storeKey.loginStatus ? null : _react2.default.createElement(
	      'p',
	      null,
	      'You need login to your own application to register new devices'
	    );
	    var button = null;
	    if (this.state.storeKey.loginStatus) {
	      button = _react2.default.createElement(
	        'a',
	        { className: 'btn btn-danger btn-lg', href: '/auth/logout' },
	        'Logout »'
	      );
	    } else {
	      button = _react2.default.createElement(
	        'a',
	        { className: 'btn btn-primary btn-lg', href: '/auth/artikcloud' },
	        'Login with your ARTIK Cloud account »'
	      );
	    }
	    return _react2.default.createElement(
	      'div',
	      ...
	      _react2.default.createElement(
	        ...
	        _react2.default.createElement(
	          'div',
	          { className: 'pull-right login' },
	          button
	        )
	      ),
	      _react2.default.createElement(
	        'div',
	        { className: 'sub-header text-right' },
	        this.state.storeKey.loginStatus ? _react2.default.createElement(
	          'span',
	          null,
	          'Logout from ARTIK Cloud when all new devices have been on-boarded.'
	        ) : _react2.default.createElement(
	          'span',
	          null,
	          'Login to ARTIK Cloud to on-board new devices.'
	        )
	      ),
	      _react2.default.createElement(
	        'div',
	        ...
	        _react2.default.createElement(
	          'div',
	          { className: 'col-sm-9' },
	          _react2.default.createElement(DevicesList, { linked: false, pollInterval: this.props.pollInterval }),
	          _react2.default.createElement(DevicesList, { linked: true, pollInterval: this.props.pollInterval })
	        ),
	        _react2.default.createElement(
	          'div',
	          { className: 'col-sm-3' },
	          _react2.default.createElement(ProxiesList, null)
	        )
	      )
	    );
	  }
	...
	_reactDom2.default.render(_react2.default.createElement(ContentBox, { pollInterval: 100000 }), document.getElementById('content'));
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = __webpack_require__.p + "description.xml";
